...
- **gsva()** A modified version of gsva function in GSVA package is presented here.
    ...
    such that they recieve high ranks and, therefore, high es scores. The es scores for 
...
We are now going to perform enrichment analysis for on Maupin's TGFb data (see `?maupin`), given a gene signature. The `maupin` data object contains a matrix containing gene expression measurements on 3 control samples and 3 TGFb samples.
...
A histogram of enrichment scores is plotted below and the density of es scores for TGFb samples is shown in red. The distribution of es scores of control samples is shown in blue. As it can be seen from the plot below, TGFb scores that are expected to be enriched in the given TGFb signature have recieved positive scores and are on the right side of the histogram, whereas the control samples are on the left side of the histogram. In addition, TGFb samples and contorl samples have been nicely separated from each other.

```{r, fig.width= 6, fig.height= 5, fig.align= "center"}
hist(es.dif, main = "enrichment scores", xlab="es")
lines(density(es.dif[,1:3]), col = "blue") # control samples
lines(density(es.dif[,4:6]), col = "red") # TGFb samples
